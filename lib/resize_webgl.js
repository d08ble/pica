...
function isArray(v) {
  return Object.prototype.toString.call(from) === '[object Array]';
}
...
function generateFragmentShader(options) {
  ...
  var pixelSizeX = 1.0 / options.texW,
      ...
      pixelSize = direction == 'horizontal' ? pixelSizeX : pixelSizeY;
  ...
  console.log('src '+options.texW+'x'+options.texH)
  console.log('dst '+options.destW+'x'+options.destH)
  console.log('scale w='+scaleW+' h='+scaleH+' w`='+1/scaleW+' h`='+1/scaleH)
  console.log('pixel px='+pixelSizeX+' py='+pixelSizeY+' px`='+1/pixelSizeX+' py`='+1/pixelSizeY)
  ...
  for (var i = 0; i < count+1; i++) {
    ...
    var px = direction == 'horizontal' ? p : 0.0;
    var py = direction == 'horizontal' ? 0.0 : p;
...
function createProgramFile(gl, vshFile, fshFile) {
...
function convolve(gl, texUnit0, texWidth, texHeight, texUnit, fsh, destW, destH, flipY) {
  var program = createProgramFile(gl, '#vsh-basic', fsh);
